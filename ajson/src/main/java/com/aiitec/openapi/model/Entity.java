...
public abstract class Entity implements Serializable , Cloneable{
  ...
  //这是我们自己的数据库框架注解
	...
	public String toString() {
		...
		String toString = "";
		try {
			toString = JSON.toJsonString(this);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return toString;
    ...
    /**
     * 两个相同属性的类的克隆 比如 Data1 和 Data2的属性完全一样，那么就可以这样
     * Data2 data2 = data1.cloneTo(Data2.class)
     * 当然这里考虑的常用的数据类型 和 继承至这个类的对象，特殊的数据类型可能不支持，但是已经达到我的要求了
     * 这个最好写在一个所有对象的父类里，要不然又有其他问题
     * @param clazz
     * @return
     ...
     */
    @SuppressWarnings("unchecked")
    public <T> T cloneTo(Class<T> clazz) throws InstantiationException, IllegalAccessException  {
        T newObj = null;
        try {
            Object entity = super.clone();
    ...
    @SuppressWarnings("unchecked")
    private <T> void copyValue (Field field, Field field2, Object entity, T newObj) throws IllegalAccessException, IllegalArgumentException, InstantiationException{
