...
public class AiiUtil {
	...
	public static String formatTime(String data) {
		...
		try {
			...
			long time = System.currentTimeMillis() - dt.getTime();
			float t = (float) time / 1000 / 60; //t是分钟
			if (t < 1) {
				newTime = "刚刚";
			}
			else if (t < 60 && t >= 1) {
				...
				if(day == 1){
					...
					if(data.length() > 10){
						newTime = data.substring(0, 10);
					} else {
						newTime = data;
					}
				}
//				newTime = (int) t / 60 / 24 + "天前";
				
			}
//			else if ((t / 60 / 24 / 30) > 1) {
//				newTime = (int) t / 60 / 24 / 30 + "个月前";
//			}
//			else if ((t / 60 / 24 / 365) > 1) {
//				newTime = (int) t / 60 / 24 / 365 + "年前";
//			}
